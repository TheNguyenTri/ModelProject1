...
public class HoaDonChiTietDAO {
    ...
    public int updateHoaDonChiTiet(String s,String s1,String s2) {
        ContentValues values = new ContentValues();
        values.put("maSach", s1);
        values.put("soLuong", s2);
        int result = db.update(TABLE_NAME, values, "maHDCT=?", new String[]{s});
        if (result == 0) {
            return -1;
        }
        return 1;
    ...
    public double getDoanhThuTheoThang() {
        double doanhThu = 0;
        String sSQL = "SELECT SUM(tongtien) from (SELECT SUM(Sach.giaBia * HoaDonChiTiet.soLuong) as 'tongtien' " + "FROM HoaDon INNER JOIN HoaDonChiTiet on HoaDon.maHoaDon = HoaDonChiTiet.maHoaDon " +
